...
const visitBaseSchema = z.object({
  ...
  responsibleUnitId: z.string().uuid().nullable().optional(),

  responsibleAccountId: z.string().uuid().nullable().optional(),
...
export const visitInputSchema = visitBaseSchema.refine(
...
export const visitUpdateInputSchema = visitBaseSchema
  .partial()
...
export const visitListQuerySchema = z.object({
  ...
  location: visitLocationSchema.optional(),
...
export const visitorSchema = visitorInputSchema.extend({
  id: z.string().uuid(),

  visitId: z.string().uuid(),

  confirmationStatus: visitorConfirmationStatusSchema,
   ...
   */
  confirmationTokenHash: z.string().nullable().optional(),

  confirmationSentAt: z.date().nullable().optional(),

  confirmationRespondedAt: z.date().nullable().optional(),

  confirmationExpiresAt: z.date().nullable().optional(),
});
...
export const visitSchema = z.object({
  ...
  responsibleUnitId: z.string().uuid().nullable(),

  responsibleAccountId: z.string().uuid().nullable(),

  status: visitStatusSchema,

  visitors: z.array(visitorSchema),

  events: z.array(visitEventSchema),
...
export const visitDashboardSchema = z.object({
  ...
  today: z.array(visitSummarySchema),

  tomorrow: z.array(visitSummarySchema),

  upcoming: z.array(visitSummarySchema),

  recentTechnicalVisits: z.array(visitSummarySchema),
});
...
export const visitorConfirmationResponseSchema = z.object({
  response: z.enum(["confirmed", "declined"]),
});

export const publicVisitConfirmationSchema = z.object({
  ...
  status: visitorConfirmationStatusSchema,
});
...
export type VisitorConfirmationStatus = z.infer<
...
export type VisitUpdateInput = z.infer<typeof visitUpdateInputSchema>;

export type VisitListQuery = z.infer<typeof visitListQuerySchema>;

export type VisitPageResult = z.infer<typeof visitPageResultSchema>;

export type VisitDecisionInput = z.infer<typeof visitDecisionInputSchema>;

export type VisitStatusInput = z.infer<typeof visitStatusInputSchema>;
...
export type VisitDashboard = z.infer<typeof visitDashboardSchema>;

export type VisitorConfirmationResponse = z.infer<
  typeof visitorConfirmationResponseSchema
>;

export type PublicVisitConfirmation = z.infer<
